...
using BFB.AWSS3Light.Abstractions.Exceptions;
...
namespace BFB.AWSS3Light.Storage.S3.Services
{
    /// <summary>
    ...
    public class ResilientS3FileStorageService : IFileStorageService
    {
        ...
        private static readonly ConcurrentDictionary<Guid, FileMetadata> _fileMetadata = new();
        ...
        private ResiliencePipeline CreateResiliencePipeline()
        {
            ...
            pipelineBuilder.AddRetry(new RetryStrategyOptions
            {
                ...
                BackoffType = _resilienceSettings.UseExponentialBackoff ? DelayBackoffType.Exponential : DelayBackoffType.Linear,                OnRetry = args =>
                {
            ...
            pipelineBuilder.AddCircuitBreaker(new CircuitBreakerStrategyOptions
            {
                ...
                BreakDuration = TimeSpan.FromSeconds(_resilienceSettings.CircuitBreakerDurationSeconds),                OnOpened = args =>
                {
        ...
        public async Task<FileUploadResponse> UploadFileAsync(FileUploadRequest request)
        {
            ...
            var objectKey = $"{DateTime.UtcNow:yyyy/MM/dd}/{fileId}/{request.File.FileName}";            return await _resiliencePipeline.ExecuteAsync(async context =>
            {
                ...
                _logger.LogInformation("File uploaded to S3 and metadata stored. FileId: {FileId}", fileId);
        ...
        {
            if (!_fileMetadata.TryGetValue(fileId, out var metadata) || !metadata.IsActive)
                throw new NotFoundException($"File with ID {fileId} not found");            return await _resiliencePipeline.ExecuteAsync(async context =>
            {
        ...
        public async Task<FileMetadata?> GetFileMetadataAsync(Guid fileId)
        {
            ...
            return _fileMetadata.TryGetValue(fileId, out var metadata) && metadata.IsActive ? metadata : null;
        }
        ...
        public async Task<bool> DeleteFileAsync(Guid fileId)
        {
                ...
                throw new NotFoundException($"File with ID {fileId} not found");            return await _resiliencePipeline.ExecuteAsync(async context =>
            {
                ...
                _logger.LogInformation("File deleted from S3 and marked inactive. FileId: {FileId}", fileId);
                return true;
            });
        }

        public async Task<string> GenerateDownloadUrlAsync(Guid fileId, int expirationMinutes = 60)
        {
            if (!_fileMetadata.TryGetValue(fileId, out var metadata) || !metadata.IsActive)
                throw new NotFoundException($"File with ID {fileId} not found");            return await _resiliencePipeline.ExecuteAsync(async context =>
            {
